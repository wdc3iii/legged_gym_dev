function visualize_trajopt_debug(nm)
...
for k = 1:size(w, 2)
    r = max(w(1, k), 0);
    rectangle('Position', [z(1, k+1, 1)-r, z(1, k+1, 2)-r, 2*r, 2*r], ...
        'Curvature', [1, 1], ...
        'EdgeColor', 'b', ...
        'LineWidth', 0.5);
end
...
plot(z(1, :, 1), z(1, :, 2), 'k', LineWidth=1);
...
subplot(2,2,2)
hold on
ci = gca().ColorOrderIndex;
plot(dyn_viol(1, :), '.-', DisplayName="Dynamics Warmstart", LineWidth=1)
...
for i = 1:size(obs_viol, 2)
    ci = gca().ColorOrderIndex;
    plot(obs_viol{i}(1, :), '.-', DisplayName=['Obstacle ' num2str(i)], LineWidth=1);
    set(gca, 'ColorOrderIndex', ci)
    gv_obs_line{i} = plot(obs_viol{i}(1, :), '.-', DisplayName=['Obstacle ' num2str(i) 'Warmstart']);
end
ci = gca().ColorOrderIndex;
gv_tub_line = plot(tube_viol(1, :), '.-', DisplayName="Tube Warmstart", LineWidth=1);
set(gca, 'ColorOrderIndex', ci)
gv_tub_line = plot(tube_viol(1, :), '.-', DisplayName="Tube");
ci = gca().ColorOrderIndex;
gv_ic_line = plot(ic_viol(1, :), '.-', DisplayName="IC Warmstart", LineWidth=1);
set(gca, 'ColorOrderIndex', ci)
...
subplot(2,2,3)
hold on
plot(squeeze(z(1, :, 1)), LineWidth=1);
plot(squeeze(z(1, :, 2)), LineWidth=1);
set(gca,'ColorOrderIndex',1)
state_x_line = plot(squeeze(z(1, :, 1)));
state_y_line = plot(squeeze(z(1, :, 2)));
legend('x warm', 'y warm', 'x', 'y')
title("State")
subplot(2,2,4)
hold on;
plot(squeeze(v(1, :, 1)), LineWidth=1);
plot(squeeze(v(1, :, 2)), LineWidth=1);
plot(w(1, :), LineWidth=1);
set(gca,'ColorOrderIndex',1)
input_x_line = plot(squeeze(v(1, :, 1)));
input_y_line = plot(squeeze(v(1, :, 2)));
tube_line = plot(w(1, :));
legend('$v_x$ warm', '$v_y$ warm', '$w$ warm', '$v_x$', '$v_y$', '$w$')
title("Input")
subplot(2, 2, 1)
