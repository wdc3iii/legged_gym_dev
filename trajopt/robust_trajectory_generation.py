...
class RobustClosedLoopTrajectoryGenerator(AbstractTrajectoryGenerator):

    def __init__(self, rom, H, N, dt_loop, device, p_dict, w_max,
                 mpc_dk=1, warm_start='nominal', nominal_ws="interpolate", track_nominal=True,
                 max_iter=200, solver_str='ipopt'):
        ...
        self.obs = p_dict['obs']
        self.robust_obs = deepcopy(self.obs)
        self.robust_obs['r'] += w_max
        ...
        self.z0 = None
        self.run_data = {}
        self.w_trajectory = np.ones((self.N, 1)) * self.w_max
        ...
        self.init_solver, self.nlp_dict, self.init_nlp_opts, self.solver, self.nlp_opts = trajopt_solver(
            self.planning_model, self.N, self.Q, self.R_nominal, self.Nobs, Qf=self.Qf, t_wall=p_dict['t_wall'],
            Rv_first=self.Rv_first, Rv_second=self.Rv_second, max_iter=self.max_iter, debug_filename=None
        )
    ...
    def step_rom_idx(self, idx, e_prev=None, increment_rom_time=False, limit_time=True):
        ...
        if (self.k.item() + 1) % self.mpc_dk == 0:
            ...
            if self.track_nominal or (self.warm_start == 'nominal' and self.z_warm is None):
                ...
                nominal_params = init_params(self.z0, self.goal, self.obs, z_cost=np.repeat(self.goal[None, :], self.N + 1, axis=0), v_cost=np.zeros((self.N, self.rom.m)))
            ...
            params = init_params(self.z0, self.goal, self.robust_obs, z_cost=z_cost, v_cost=v_cost)

            x_init = init_decision_var(self.z_warm, self.v_warm)
            ...
            self.trajectory, self.v_trajectory = extract_solution(sol, self.N, self.planning_model.n, self.planning_model.m)
            ...
            data["sol_w"] = np.ones((self.N, 1)) * self.w_max
